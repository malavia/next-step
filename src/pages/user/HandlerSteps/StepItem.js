...
import { Lock, Unlock, Trash, Plus, Loader } from 'lucide-react'; // Import du loader
...
const StepItem = ({ 
  ...
  deleteSubStep,
  generateSubStep,
  isGeneratingSubStep // Nouvelle prop pour indiquer si la génération est en cours
}) => {
  return (
    <div       
      ...
      <div className="p-4 space-y-2">
        ...
        <div className="flex gap-2 mt-4">
          ...
          <button 
            ...
            disabled={isGeneratingSubStep} // Désactiver le bouton pendant la génération
          >
            {isGeneratingSubStep ? (
              <Loader className="h-4 w-4 animate-spin" /> // Loader pendant la génération
            ) : (
              <Wand2 className="h-4 w-4" />
            )}
            {isGeneratingSubStep ? 'Génération en cours...' : 'Générer une sous-étape'}
          </button>
        </div>
      </div>
    </div>
